...
- **"Why PBKDF2 + Fernet instead of raw AES?"** — Fernet already bundles
  AES with authenticated encryption (HMAC) and proper IV handling, which
  avoids common mistakes people make implementing raw AES by hand
  (like reusing IVs). PBKDF2 with 200k iterations makes brute-forcing the
  password computationally expensive.
- **"What stops an attacker from just deleting the Gist?"** — the private
  Gist is created under the app owner's own GitHub token, not exposed to
  the attacker in any response; the attacker only ever sees the encrypted
  local file or the honeypot decoy, never the backup location.
- **"What if the attacker is the real owner and just forgot their
  password?"** — by design, there's no backdoor: if you lose your master
  password, the file is unrecoverable. This is a deliberate trade-off
  (same as VeraCrypt/BitLocker) — a vault that could be recovered without
  the password wouldn't be secure against anyone else either.
- **"Why GitHub Gist and not S3/Drive?"** — for a hackathon demo, it needs
  zero paid infrastructure and a 2-minute setup (one token). The upload
  function (`upload_to_backup`) is a single, isolated function — swapping
  it for S3/Drive/Azure Blob later is a contained change.
